-- Inserts the official mission catalog from Documentos/Missoes PROCEL.pdf.
-- Historical/custom missions and their dependent rows are removed so the
-- catalog contains only the official missions.

with official_missions(id, titulo, descricao, xp) as (
    values
...
select id, titulo, descricao, 'Individual', xp, true, now()
from official_missions v
where not exists (
    select 1
    from missao m
    where m.id = v.id
...
create temporary table tmp_official_mission_ids (
    id uuid primary key
) on commit drop;

insert into tmp_official_mission_ids (id)
values
...
delete from evento_janela_evidencia eje
where exists (
    select 1
    from evento_janela_avaliacao ej
    join evento_definicao ed on ed.id = ej.evento_definicao_id
    where eje.evento_janela_avaliacao_id = ej.id
      and ed.missao_id not in (select id from tmp_official_mission_ids)
);

delete from atividade_evento ae
where exists (
    select 1
    from atividade a
    where ae.atividade_id = a.id
      and a.missao_id not in (select id from tmp_official_mission_ids)
)
or exists (
    select 1
    from evento_ocorrencia eo
    join evento_definicao ed on ed.id = eo.evento_definicao_id
    where ae.evento_ocorrencia_id = eo.id
      and ed.missao_id not in (select id from tmp_official_mission_ids)
);

delete from xp_lancamento xl
where xl.missao_id not in (select id from tmp_official_mission_ids)
   or exists (
       select 1
       from atividade a
       where xl.atividade_id = a.id
         and a.missao_id not in (select id from tmp_official_mission_ids)
   )
   or exists (
       select 1
       from evento_ocorrencia eo
       join evento_definicao ed on ed.id = eo.evento_definicao_id
       where xl.evento_ocorrencia_id = eo.id
         and ed.missao_id not in (select id from tmp_official_mission_ids)
   );

delete from evento_ocorrencia_evidencia eoe
where exists (
    select 1
    from evento_ocorrencia eo
    join evento_definicao ed on ed.id = eo.evento_definicao_id
    where eoe.evento_ocorrencia_id = eo.id
      and ed.missao_id not in (select id from tmp_official_mission_ids)
);

delete from evento_janela_avaliacao ej
where exists (
    select 1
    from evento_definicao ed
    where ej.evento_definicao_id = ed.id
      and ed.missao_id not in (select id from tmp_official_mission_ids)
);

delete from evento_ocorrencia eo
where exists (
    select 1
    from evento_definicao ed
    where eo.evento_definicao_id = ed.id
      and ed.missao_id not in (select id from tmp_official_mission_ids)
);

delete from evento_condicao ec
where exists (
    select 1
    from evento_definicao ed
    where ec.evento_definicao_id = ed.id
      and ed.missao_id not in (select id from tmp_official_mission_ids)
);

delete from evento_definicao ed
where ed.missao_id not in (select id from tmp_official_mission_ids);

delete from atividade a
where a.missao_id not in (select id from tmp_official_mission_ids);

update missao m
set parent_id = null
where m.parent_id is not null
  and m.parent_id not in (select id from tmp_official_mission_ids);

delete from missao m
where m.id not in (select id from tmp_official_mission_ids);
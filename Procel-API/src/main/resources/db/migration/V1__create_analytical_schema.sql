...
create table atividade (
    ...
    status varchar(30) not null check ((status in ('PENDENTE','EM_ANDAMENTO','CONCLUIDA','EXPIRADA','CANCELADA'))),
    pessoa_id varchar(80) not null,
    primary key (id),
    constraint uk_atividade_pessoa_modelo unique (pessoa_id, missao_id)
);
...
create index ix_atividade_pessoa_status
   on atividade (pessoa_id, status);

create index ix_atividade_missao
   on atividade (missao_id);

create index ix_atividade_assigned_at
